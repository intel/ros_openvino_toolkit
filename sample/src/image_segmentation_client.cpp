...
int main(int argc, char **argv) {
  ...
  ros::ServiceClient client = n.serviceClient<people_msgs::ObjectsInMasksSrv>(
  ...
  if (client.call(srv)) {
    ...
    for (unsigned int i = 0;
         i < srv.response.segmentation.objects_vector.size(); i++) {
      std::stringstream ss;
      ...
      ROS_INFO("location: (%d, %d, %d, %d)",
               srv.response.segmentation.objects_vector[i].roi.x_offset,
