...
int main(int argc, char **argv) {
  ...
  ros::ServiceClient client =
      n.serviceClient<people_msgs::PeopleSrv>("/openvino_toolkit/service");
  ...
  if (client.call(srv)) {
    ...
    if (srv.response.persons.emotions.size() == 0 &&
        srv.response.persons.agegenders.size() == 0 &&
    ...
    for (unsigned int i = 0; i < srv.response.persons.faces.size(); i++) {
      ...
      ROS_INFO("location: (%d, %d, %d, %d)",
               srv.response.persons.faces[i].roi.x_offset,
               srv.response.persons.faces[i].roi.y_offset,
               srv.response.persons.faces[i].roi.width,
      ...
      ROS_INFO("Yaw, Pitch and Roll for head pose is: (%f, %f, %f),",
               srv.response.persons.headposes[i].yaw,
               srv.response.persons.headposes[i].pitch,
