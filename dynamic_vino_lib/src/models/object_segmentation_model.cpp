  ...
  for (const auto &inputInfoItem : input_info_) {
    ...
    if (inputInfoItem.second->getTensorDesc().getDims().size() == 2) {
      InferenceEngine::Blob::Ptr input =
          engine->getRequest()->GetBlob(inputInfoItem.first);
...
bool Models::ObjectSegmentationModel::matToBlob(
  ...
  if (!is_dense) {
    slog::err << "Doesn't support conversion from not dense cv::Mat."
              << slog::endl;
    return false;
  }
  ...
  auto shared_blob =
      InferenceEngine::make_shared_blob<uint8_t>(tDesc, orig_image.data);
...
bool Models::ObjectSegmentationModel::updateLayerProperty(
  ...
  if (inputShapes.size() != 1) {
    ...
    slog::warn << "This inference sample should have only one input, but we got"
               << std::to_string(inputShapes.size()) << "inputs" << slog::endl;
    return false;
  }

  InferenceEngine::SizeVector &in_size_vector = inputShapes.begin()->second;
  slog::debug << "channel size" << in_size_vector[1] << "dimensional"
              << in_size_vector.size() << slog::endl;
  if (in_size_vector.size() != 4 || in_size_vector[1] != 3) {
    // throw std::runtime_error("3-channel 4-dimensional model's input is
    // expected");
    slog::warn
        << "3-channel 4-dimensional model's input is expected, but we got "
        << std::to_string(in_size_vector[1]) << " channels and "
        << std::to_string(in_size_vector.size()) << " dimensions."
        << slog::endl;
    return false;
  }
  in_size_vector[0] = 1;
  ...
  inputInfo.getPreProcess().setResizeAlgorithm(
  ...
  if (outputsDataMap.size() != 1) {
    ...
    slog::warn
        << "This inference sample should have only one output, but we got"
  ...
  const InferenceEngine::SizeVector &outSizeVector =
