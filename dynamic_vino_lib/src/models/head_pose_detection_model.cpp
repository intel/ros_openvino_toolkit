...
bool Models::HeadPoseDetectionModel::updateLayerProperty(
  ...
  for (const std::string &outName :
       {output_angle_r_, output_angle_p_, output_angle_y_}) {
    if (output_info_map.find(outName) == output_info_map.end()) {
      throw std::logic_error("There is no " + outName +
                             " output in Head Pose Estimation network");
    } else {
