...
/**
 * @brief a realization file with declaration of LicensePlateDetectionModel
 * class
...
bool Models::LicensePlateDetectionModel::updateLayerProperty(
  ...
  if (input_info_map.size() != 2) {
    throw std::logic_error(
        "Vehicle Attribs topology should have only two inputs");
  }
  auto sequence_input = (++input_info_map.begin());
  if (sequence_input->second->getTensorDesc().getDims()[0] !=
      getMaxSequenceSize()) {
    throw std::logic_error(
        "License plate detection max sequence size dismatch");
  }
  InferenceEngine::OutputsDataMap output_info_map(
      net_reader->getNetwork().getOutputsInfo());
  if (output_info_map.size() != 1) {
    throw std::logic_error(
        "Vehicle Attribs Network expects networks having one output");
  }
