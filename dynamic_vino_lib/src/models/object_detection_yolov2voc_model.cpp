...
bool Models::ObjectDetectionYolov2Model::updateLayerProperty(
  ...
  if (input_info_map.size() != 1) {
    slog::warn << "This model seems not Yolo-like, which has only one input, "
                  "but we got "
  ...
  if (output_info_map.size() != 1) {
    slog::warn << "This model seems not Yolo-like! We got "
               << std::to_string(output_info_map.size())
  ...
  slog::info << "Checking Object Detection output ... Name="
             << output_info_map.begin()->first << slog::endl;

  const InferenceEngine::CNNLayerPtr output_layer =
      net_reader->getNetwork().getLayerByName(
          output_info_map.begin()->first.c_str());
  ...
  if (output_layer == nullptr ||
    ...
    slog::warn << "This model's output layer ("
               << output_info_map.begin()->first
  ...
  slog::info << "Checking Object Detection output ... num_classes="
  ...
  if (object_size != 33) {
    slog::warn << "This model is NOT Yolo-like, whose output data for each "
                  "detected object"
               << "should have 7 dimensions, but was "
               << std::to_string(object_size) << slog::endl;
    return false;
  }
  setObjectSize(object_size);

  if (output_dims.size() != 2) {
    slog::warn << "This model is not Yolo-like, output dimensions shoulld be "
                  "2, but was"
  ...
  printAttribute();
  slog::info << "This model is Yolo-like, Layer Property updated!"
             << slog::endl;
  return true;
}
...
bool Models::ObjectDetectionYolov2Model::enqueue(
    const std::shared_ptr<Engines::Engine> &engine, const cv::Mat &frame,
...
bool Models::ObjectDetectionYolov2Model::matToBlob(
  ...
  if (engine == nullptr) {
    slog::err << "A frame is trying to be enqueued in a NULL Engine."
              << slog::endl;
    return false;
  }
  ...
  float resize_ratio =
      static_cast<float>(IH) / static_cast<float>(std::max(imw, imh));
  ...
  for (int row = 0; row < imh; row++) {
    for (int col = 0; col < imw; col++) {
      for (int ch = 0; ch < 3; ch++) {
        resizedImg.at<cv::Vec3f>(dy + row, dx + col)[ch] =
            image.at<cv::Vec3f>(row, col)[ch];
      }
    }
  }
...
bool Models::ObjectDetectionYolov2Model::fetchResults(
  ...
  try {
    ...
    std::vector<float> anchors = {0.572730, 0.677385, 1.874460, 2.062530,
                                  3.338430, 5.474340, 7.882820, 3.527780,
                                  9.770520, 9.168280};
    ...
    for (int i = 0; i < side_square; ++i) {
      ...
      for (int n = 0; n < num; ++n) {
        int obj_index =
            getEntryIndex(side, coords, classes, n * side * side + i, coords);
        int box_index =
            getEntryIndex(side, coords, classes, n * side * side + i, 0);
        ...
        float x = (col + detections[box_index + 0 * side_square]) / side *
                  input_width;
        float y = (row + detections[box_index + 1 * side_square]) / side *
                  input_height;
        float height = std::exp(detections[box_index + 3 * side_square]) *
                       anchors[2 * n + 1] / side * input_height;
        float width = std::exp(detections[box_index + 2 * side_square]) *
                      anchors[2 * n] / side * input_width;

        for (int j = 0; j < classes; ++j) {
          int class_index = getEntryIndex(side, coords, classes,
          ...
          cv::Rect r(x_min_resized, y_min_resized, width_resized,
                     height_resized);
          Result result(r);
          // result.label_ = j;
          std::string label = j < labels.size()
                                  ? labels[j]
...
int Models::ObjectDetectionYolov2Model::getEntryIndex(int side, int lcoords,
                                                      int lclasses,
