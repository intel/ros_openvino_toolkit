...
bool Models::ObjectDetectionSSDModel::fetchResults(
  ...
  slog::debug << "fetching Infer Resulsts from the given SSD model"
              << slog::endl;
  if (engine == nullptr) {
  ...
  for (int i = 0; i < max_proposal_count; i++) {
    ...
    if (image_id < 0) {
      // slog::info << "Found objects: " << i << "|" << results.size() <<
      // slog::endl;
      break;
    }
    ...
    r.width = static_cast<int>(
        detections[i * object_size + 5] * frame_size.width - r.x);
    r.height = static_cast<int>(
        detections[i * object_size + 6] * frame_size.height - r.y);
    ...
    if (confidence <=
        confidence_thresh /* || r.x == 0 */) { // why r.x needs to be checked?
      continue;
    }
...
bool Models::ObjectDetectionSSDModel::updateLayerProperty(
  ...
  if (input_info_map.size() != 1) {
    slog::warn << "This model seems not SSDNet-like, SSDnet has only one "
                  "input, but we got "
  ...
  const InferenceEngine::SizeVector input_dims =
      input_info->getTensorDesc().getDims();
  ...
  if (object_size != 7) {
    slog::warn << "This model is NOT SSDNet-like, whose output data for each "
                  "detected object"
  ...
  if (output_dims.size() != 4) {
    slog::warn << "This model is not SSDNet-like, output dimensions shoulld be "
                  "4, but was"
  ...
  printAttribute();
  slog::info << "This model is SSDNet-like, Layer Property updated!"
             << slog::endl;
  return true;
}
