...
// ObjectDetection
dynamic_vino_lib::ObjectDetectionYolov2voc::ObjectDetectionYolov2voc(double show_output_thresh)
...
dynamic_vino_lib::ObjectDetectionYolov2voc::~ObjectDetectionYolov2voc() = default;

void dynamic_vino_lib::ObjectDetectionYolov2voc::loadNetwork(
         std::shared_ptr<Models::ObjectDetectionModel> network) {
  valid_model_ = std::dynamic_pointer_cast<Models::ObjectDetectionYolov2vocModel>(network);
...
bool dynamic_vino_lib::ObjectDetectionYolov2voc::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
...
bool dynamic_vino_lib::ObjectDetectionYolov2voc::matToBlob(
...
bool dynamic_vino_lib::ObjectDetectionYolov2voc::submitRequest() {
  return dynamic_vino_lib::BaseInference::submitRequest();
}

bool dynamic_vino_lib::ObjectDetectionYolov2voc::fetchResults()
{
...
double dynamic_vino_lib::ObjectDetectionYolov2voc::IntersectionOverUnion(const Result &box_1, const Result &box_2) {
...
int dynamic_vino_lib::ObjectDetectionYolov2voc::getEntryIndex(int side, int lcoords, int lclasses, int location, int entry) {
    int n = location / (side * side);
    int loc = location % (side * side);
    return n * side * side * (lcoords + lclasses + 1) + entry * side * side + loc;
}

const int dynamic_vino_lib::ObjectDetectionYolov2voc::getResultsLength() const {
  return static_cast<int>(results_.size());
}

const dynamic_vino_lib::Result*
dynamic_vino_lib::ObjectDetectionYolov2voc::getLocationResult(int idx) const {
  return &(results_[idx]);
}

const std::string dynamic_vino_lib::ObjectDetectionYolov2voc::getName() const {
  return valid_model_->getModelName();
}

const void dynamic_vino_lib::ObjectDetectionYolov2voc::observeOutput(
...
const std::vector<cv::Rect> dynamic_vino_lib::ObjectDetectionYolov2voc::getFilteredROIs(
