...
// Tracker
dynamic_vino_lib::Tracker::Tracker(int max_record_size,
                                   double same_track_thresh,
                                   double new_track_thresh)
    : max_record_size_(max_record_size), same_track_thresh_(same_track_thresh),
      new_track_thresh_(new_track_thresh) {}

int dynamic_vino_lib::Tracker::processNewTrack(
...
double dynamic_vino_lib::Tracker::findMostSimilarTrack(
  ...
  most_similar_id = -1;
  for (auto iter = recorded_tracks_.begin(); iter != recorded_tracks_.end();
       iter++) {
...
double
dynamic_vino_lib::Tracker::calcSimilarity(const std::vector<float> &feature_a,
