...
dynamic_vino_lib::ObjectSegmentationResult::ObjectSegmentationResult(const cv::Rect& location) : Result(location)
{
}
...
void dynamic_vino_lib::ObjectSegmentation::loadNetwork(const std::shared_ptr<Models::ObjectSegmentationModel> network)
{
 ...
 */
bool dynamic_vino_lib::ObjectSegmentation::enqueue_for_one_input(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
...
bool dynamic_vino_lib::ObjectSegmentation::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (width_ == 0 && height_ == 0)
  {
    width_ = frame.cols;
...
const dynamic_vino_lib::Result* dynamic_vino_lib::ObjectSegmentation::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void dynamic_vino_lib::ObjectSegmentation::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr)
  {
...
dynamic_vino_lib::ObjectSegmentation::getFilteredROIs(const std::string filter_conditions) const
{
