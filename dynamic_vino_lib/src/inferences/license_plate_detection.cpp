...
dynamic_vino_lib::LicensePlateDetectionResult::LicensePlateDetectionResult(const cv::Rect& location) : Result(location)
{
}

// LicensePlateDetection
dynamic_vino_lib::LicensePlateDetection::LicensePlateDetection() : dynamic_vino_lib::BaseInference()
{
}
...
bool dynamic_vino_lib::LicensePlateDetection::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (getEnqueuedNum() == 0)
  {
    results_.clear();
...
const dynamic_vino_lib::Result* dynamic_vino_lib::LicensePlateDetection::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void dynamic_vino_lib::LicensePlateDetection::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr)
  {
...
dynamic_vino_lib::LicensePlateDetection::getFilteredROIs(const std::string filter_conditions) const
{
