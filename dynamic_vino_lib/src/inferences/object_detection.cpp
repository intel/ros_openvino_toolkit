...
bool dynamic_vino_lib::ObjectDetection::fetchResults() {
  ...
  return (valid_model_ != nullptr) &&
         valid_model_->fetchResults(getEngine(), results_, show_output_thresh_,
                                    enable_roi_constraint_);
}
...
double dynamic_vino_lib::ObjectDetection::calcIoU(const cv::Rect &box_1,
                                                  const cv::Rect &box_2) {
