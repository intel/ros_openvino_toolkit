...
bool dynamic_vino_lib::BaseFilter::isPriorTo(const std::string &operator1,
...
bool dynamic_vino_lib::BaseFilter::strToBool(const std::string &value) {
  if (!value.compare("true")) {
    ...
    slog::err << "Invalid string: " << value << " for bool conversion!"
              << slog::endl;
  }
  return false;
}
...
bool dynamic_vino_lib::BaseFilter::logicOperation(const std::string &logic1,
                                                  const std::string &op,
  ...
  if (!op.compare("&&")) {
    ...
    slog::err << "Invalid operator: " << op << " for logic operation!"
              << slog::endl;
    return false;
  }
}

bool dynamic_vino_lib::BaseFilter::stringCompare(const std::string &candidate,
                                                 const std::string &op,
  ...
  if (!op.compare("==")) {
    ...
    slog::err << "Invalid operator " << op << " for label comparsion"
              << slog::endl;
    return false;
  }
}

bool dynamic_vino_lib::BaseFilter::floatCompare(float candidate,
                                                const std::string &op,
                                                float target) {
  if (!op.compare("<=")) {
    ...
    slog::err << "Invalid operator " << op << " for confidence comparsion"
              << slog::endl;
    return false;
  }
}

float dynamic_vino_lib::BaseFilter::stringToFloat(
    const std::string &candidate) {
  float result = 0;
  try {
    result = std::stof(candidate);
  } catch (...) {
    slog::err << "Failed when converting string " << candidate << " to float"
              << slog::endl;
  }
  return result;
}
...
dynamic_vino_lib::BaseFilter::split(const std::string &filter_conditions) {
  std::vector<std::string> seperators;
  seperators.insert(seperators.end(), relation_operators_.begin(),
                    relation_operators_.end());
  seperators.insert(seperators.end(), logic_operators_.begin(),
                    logic_operators_.end());
  ...
  if (last_pos != pos) {
    infix_conditions.push_back(
        filter_conditions.substr(last_pos, pos - last_pos));
  }
