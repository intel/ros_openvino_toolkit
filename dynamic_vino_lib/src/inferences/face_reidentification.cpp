...
dynamic_vino_lib::FaceReidentification::FaceReidentification(
    double match_thresh)
...
bool dynamic_vino_lib::FaceReidentification::fetchResults() {
  ...
  for (int i = 0; i < getResultsLength(); i++) {
    std::vector<float> new_face =
        std::vector<float>(output_values + result_length * i,
                           output_values + result_length * (i + 1));
    std::string face_id =
        "No." + std::to_string(face_tracker_->processNewTrack(new_face));
...
dynamic_vino_lib::FaceReidentification::getFilteredROIs(
