...
dynamic_vino_lib::FaceReidentificationResult::FaceReidentificationResult(const cv::Rect& location) : Result(location)
{
}

// FaceReidentification
dynamic_vino_lib::FaceReidentification::FaceReidentification(double match_thresh) : dynamic_vino_lib::BaseInference()
{
...
bool dynamic_vino_lib::FaceReidentification::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (getEnqueuedNum() == 0)
  {
    results_.clear();
...
const dynamic_vino_lib::Result* dynamic_vino_lib::FaceReidentification::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void dynamic_vino_lib::FaceReidentification::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr)
  {
...
dynamic_vino_lib::FaceReidentification::getFilteredROIs(const std::string filter_conditions) const
{
