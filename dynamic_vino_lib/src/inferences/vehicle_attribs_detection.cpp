...
bool dynamic_vino_lib::VehicleAttribsDetection::fetchResults() {
  ...
  const float *color_values =
      request->GetBlob(color_name)->buffer().as<float *>();
  const float *type_values =
      request->GetBlob(type_name)->buffer().as<float *>();
  for (int i = 0; i < getResultsLength(); i++) {
    auto color_id =
        std::max_element(color_values, color_values + 7) - color_values;
...
dynamic_vino_lib::VehicleAttribsDetection::getFilteredROIs(
