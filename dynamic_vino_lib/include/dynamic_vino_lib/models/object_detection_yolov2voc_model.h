...
public:
  ...
  bool enqueue(const std::shared_ptr<Engines::Engine> &engine,
               const cv::Mat &frame, const cv::Rect &input_frame_loc) override;

  bool matToBlob(const cv::Mat &orig_image, const cv::Rect &,
...
protected:
  int getEntryIndex(int side, int lcoords, int lclasses, int location,
                    int entry);
