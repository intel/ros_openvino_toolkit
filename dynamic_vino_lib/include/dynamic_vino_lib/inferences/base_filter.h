...
public:
   ...
   */
  bool logicOperation(const std::string &, const std::string &,
                      const std::string &);
   ...
   */
  static bool stringCompare(const std::string &, const std::string &,
                            const std::string &);
...
/**
 * @brief A macro to decide whether a given result satisfies the filter
 * condition.
      ...
      if (!isRelationOperator(elem) && !isLogicOperator(elem)) {               \
        ...
        try {                                                                  \
          ...
          if (key_to_function.count(str2)) {                                   \
            result_stack.push(                                                 \
                boolToStr(key_to_function[str2](result, elem, str1)));         \
          } else {                                                             \
