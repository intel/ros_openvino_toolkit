...
void matU8ToBlob(const cv::Mat &orig_image, InferenceEngine::Blob::Ptr &blob,
  ...
  if (width != (size_t)orig_image.size().width ||
      height != (size_t)orig_image.size().height) {
...
public:
   ...
   */
  inline int getEnqueuedNum() const { return enqueued_frames_; }
  ...
  inline void setMaxBatchSize(int max) { max_batch_size_ = max; }
  ...
  virtual void
  observeOutput(const std::shared_ptr<Outputs::BaseOutput> &output) = 0;
  ...
  virtual const std::vector<cv::Rect>
  getFilteredROIs(const std::string filter_conditions) const = 0;
