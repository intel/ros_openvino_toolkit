...
public:
  ...
  std::string getAttributes() const { return attributes_; }
  bool getMaleProbability() const { return male_probability_; }
  cv::Point2f getTopLocation() const { return top_point_; }
  cv::Point2f getBottomLocation() const { return bottom_point_; }
 ...
 * @brief Class to load person attributes detection model and perform person
...
public:
  ...
  getFilteredROIs(const std::string filter_conditions) const override;
...
private:
  ...
  const std::vector<std::string> net_attributes_ = {
      "is male",         "has_bag",       "has_backpack", "has hat",
