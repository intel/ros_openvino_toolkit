...
private:
   ...
   */
  double findMostSimilarTrack(const std::vector<float> &feature,
                              int &most_similar_id);
  /**
  ...
  /**
   * @brief Add a new track to the recorded tracks, remove oldest track if
   * needed.
   ...
   */
  double calcSimilarity(const std::vector<float> &feature_a,
