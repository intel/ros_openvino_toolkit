...
void operator>>(const YAML::Node &node, int &val) { val = node.as<int>(); }

void operator>>(const YAML::Node &node, float &val) { val = node.as<float>(); }
...
void ParamManager::print() const {
  ...
  for (auto &pipeline : pipelines_) {
    ...
    for (auto &infer : pipeline.infers) {
      ...
      slog::info << "\t\tConfidence_threshold: " << infer.confidence_threshold
                 << slog::endl;
      slog::info << "\t\tEnable_roi_constraint: " << infer.enable_roi_constraint
                 << slog::endl;
    }
...
ParamManager::PipelineRawData
ParamManager::getPipeline(const std::string &name) const {
  for (auto &p : pipelines_) {
    if (p.name == name) {
      return p;
    }
