...
int main(int argc, char** argv)
{
  ...
  if (client.call(srv))
  {
    ...
    {
      ROS_INFO("%d: object: %s", i, srv.response.persons.faces[i].object.object_name.c_str());
      ROS_INFO("prob: %f", srv.response.persons.faces[i].object.probability);
      ROS_INFO("location: (%d, %d, %d, %d)", srv.response.persons.faces[i].roi.x_offset,
               srv.response.persons.faces[i].roi.y_offset, srv.response.persons.faces[i].roi.width,
               srv.response.persons.faces[i].roi.height);
      ROS_INFO("Emotions: %s", srv.response.persons.emotions[i].emotion.c_str());
      ROS_INFO("Age: %f, Gender: %s", srv.response.persons.agegenders[i].age,
               srv.response.persons.agegenders[i].gender.c_str());
      ROS_INFO("Yaw, Pitch and Roll for head pose is: (%f, %f, %f),", srv.response.persons.headposes[i].yaw,
               srv.response.persons.headposes[i].pitch, srv.response.persons.headposes[i].roll);
    }
  }
  else
  {
