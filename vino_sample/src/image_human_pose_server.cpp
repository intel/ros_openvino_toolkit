...
* \file src/image_human_pose_server.cpp
*/
...
{
  ros::init(argc, argv, "human_pose_server");
  
  if (!parseAndCheckCommandLine(argc, argv))  return 0;

  ros::param::param<std::string>("~param_file", FLAGS_config, "/param/pipeline_pose.yaml");
  ...
  auto node = std::make_shared<vino_service::FrameProcessingServer
    <vino_people_msgs::HumanPoseSrv>>(service_name, FLAGS_config);
  
  slog::info << "Waiting for service request..." << slog::endl;
  ros::spin();
  slog::info << "--------------End of Execution--------------" << FLAGS_config << slog::endl;
